...
simd::float_4 getBlockExpressions(simd::float_4 exprIn,
...
template<unsigned int SIZE> struct Pattern {
    ...
    simd::float_4 attrMutaMultipliers[STEP_ATTRS_TOTAL] = {simd::float_4(0.5f),
                                                           simd::float_4(0.1f),
                                                           simd::float_4(0.02f),
                                                           simd::float_4(1.0f),
                                                           simd::float_4(1.0f),
    ...
    void findStepsForPhase(float phase, bool globalGate)
    {
        ...
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            ...
            this->hits[blockIdx]
                = ((inMod <= phase) ^ (phase < outMod) ^ (inMod < outMod))
    ...
    void findMonoStep()
    {
        ...
        for (unsigned int blockOffset = 0; blockOffset < SIZE / BLOCK_SIZE; blockOffset++) {
            ...
            for (unsigned int stepInBlockOffset = 0; stepInBlockOffset < BLOCK_SIZE;
                 stepInBlockOffset++) {
    ...
    void applyMutations(unsigned int attrIdx, unsigned int blockIdx)
    {
        ...
        this->stepBasesMutated[attrIdx][blockIdx]
            = simd::clamp(this->stepBases[attrIdx][blockIdx] + this->stepMutas[attrIdx][blockIdx],
                          bounds.first,
                          bounds.second);
    }
    ...
    {
        this->stepInsComputed[blockIdx]
            = eucMod(this->stepIns[blockIdx] + this->stepBases[StepAttr::STEP_SHIFT][blockIdx]
                         + this->shift + this->globalShift,
                     1.f);

        this->stepOutsComputed[blockIdx]
            = eucMod(this->stepInsComputed[blockIdx]
                         + this->stepBases[StepAttr::STEP_LEN][blockIdx] * this->globalLen,
                     1.f);

        this->stepMutaInsComputed[blockIdx] = eucMod(
            this->stepIns[blockIdx] + this->stepBasesMutated[StepAttr::STEP_SHIFT][blockIdx]
                + this->shift + this->globalShift,
            1.f);
    ...
    json_t *dataToJson()
    {
        ...
        for (unsigned int stepIdx = 0; stepIdx < SIZE; stepIdx++) {
            ...
            json_object_set_new(stepJ,
                                "gate",
                                json_boolean(bool(simd::movemask(this->stepGates[blockIdx])
                                                  & (1 << stepInBlockIdx))));
            ...
            for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
                ...
                json_object_set_new(attrJ,
                                    "base",
                                    json_real(this->stepBases[attrIdx][blockIdx][stepInBlockIdx]));
                json_object_set_new(attrJ,
                                    "mutation",
                                    json_real(this->stepMutas[attrIdx][blockIdx][stepInBlockIdx]));
        ...
        for (unsigned int stepIdx = 0; stepIdx < SIZE; stepIdx++) {
            ...
            for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
                ...
                this->stepBases[attrIdx][blockIdx][stepInBlockIdx]
                    = json_number_value(json_object_get(attrJ, "base"));
    ...
    bool isClean()
    {
        ...
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                ...
                if (simd::movemask(this->stepBases[attrIdx][blockIdx]
                                   != this->stepAttrDefaults[attrIdx])
    ...
    void randomizeReso() { this->resolution = 1 + std::round(99 * random::uniform()); }

    void resetResolution() { this->resolution = SIZE; }

    void quantize()
    {
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            this->stepBases[StepAttr::STEP_SHIFT][blockIdx]
                = simd::trunc(this->stepBases[StepAttr::STEP_SHIFT][blockIdx] * SIZE * 2) / SIZE
                  * 2;
            this->stepMutas[StepAttr::STEP_SHIFT][blockIdx]
                = simd::trunc(this->stepMutas[StepAttr::STEP_SHIFT][blockIdx] * SIZE * 2) / SIZE
                  * 2;
...
private:
    void shiftBy(int delta)
    {
        ...
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            ...
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(this->stepBases[attrIdx][blockIdx].s,
                          this->stepBases[attrIdx][blockIdx].s + BLOCK_SIZE,
                          bases + blockIdx * BLOCK_SIZE);
            }

            std::rotate(&bases[0], &bases[targetShift], &bases[SIZE]);
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(bases + blockIdx * BLOCK_SIZE,
                          bases + (blockIdx + 1) * BLOCK_SIZE,
                          this->stepBases[attrIdx][blockIdx].s);
            }

            float mutas[SIZE] = {0.f};
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(this->stepMutas[attrIdx][blockIdx].s,
                          this->stepMutas[attrIdx][blockIdx].s + BLOCK_SIZE,
                          mutas + blockIdx * BLOCK_SIZE);
            }

            std::rotate(&mutas[0], &mutas[targetShift], &mutas[SIZE]);
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(mutas + blockIdx * BLOCK_SIZE,
                          mutas + (blockIdx + 1) * BLOCK_SIZE,
        ...
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            std::copy(this->stepGates[blockIdx].s,
                      this->stepGates[blockIdx].s + BLOCK_SIZE,
                      gates + blockIdx * BLOCK_SIZE);
        }

        std::rotate(&gates[0], &gates[targetShift], &gates[SIZE]);
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            std::copy(gates + blockIdx * BLOCK_SIZE,
                      gates + (blockIdx + 1) * BLOCK_SIZE,
                      this->stepGates[blockIdx].s);
            this->recalcInOuts(blockIdx);
        }
    }

public:
    void shiftLeft() { this->shiftBy(1); }

    void shiftRight() { this->shiftBy(-1); }

    float getShift() { return this->shift / this->baseStepLen; }
    ...
    void resetLenghts()
    {
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            this->stepBases[StepAttr::STEP_LEN][blockIdx]
                = this->stepAttrDefaults[StepAttr::STEP_LEN];
    ...
    void mutateBlock(unsigned int blockIdx, simd::float_4 mask, float factorScalar)
    {
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            ...
            *vctrs = simd::ifelse(mask,
                                  simd::clamp(*vctrs + (rands * vectorLifeFactor)
                                                  - (overflowCaution * antiOverflowFactor),
                                              -1.f,
                                              1.f),
                                  *vctrs * 0.98f);

            simd::float_4 newMutas = simd::ifelse(factor < 0.f,
    ...
    {
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            this->stepMutas[attrIdx][blockIdx]
                = simd::ifelse(mask, 0.f, this->stepMutas[attrIdx][blockIdx]);
            this->applyMutations(attrIdx, blockIdx);
        }
        this->recalcInOuts(blockIdx);
    }
    ...
    void reverse()
    {
        ...
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            ...
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(newBases + blockIdx * BLOCK_SIZE,
                          newBases + (blockIdx + 1) * BLOCK_SIZE,
                          this->stepBases[attrIdx][blockIdx].s);
            }
            ...
            for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
                std::copy(newMutas + blockIdx * BLOCK_SIZE,
                          newMutas + (blockIdx + 1) * BLOCK_SIZE,
                          this->stepMutas[attrIdx][blockIdx].s);
            }
        }
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            ...
            this->stepBases[StepAttr::STEP_SHIFT][blockIdx]
                = this->baseStepLen - this->stepBases[StepAttr::STEP_SHIFT][blockIdx]
                  - this->stepBases[StepAttr::STEP_LEN][blockIdx];
            this->stepMutas[StepAttr::STEP_SHIFT][blockIdx]
                = -this->stepMutas[StepAttr::STEP_SHIFT][blockIdx];
    ...
    {
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE; blockIdx++) {
            this->stepBases[StepAttr::STEP_VALUE][blockIdx]
                = -this->stepBases[StepAttr::STEP_VALUE][blockIdx];
            this->stepMutas[StepAttr::STEP_VALUE][blockIdx]
                = -this->stepMutas[StepAttr::STEP_VALUE][blockIdx];

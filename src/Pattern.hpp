...
struct Pattern {
    ...
    void findStepsForPhase(float phase, bool globalGate) {
        ...
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE;
             blockIdx++) {
            ...
            this->hits[blockIdx] =
                ...
                & (this->stepGates[blockIdx]
                   ^ createMask(globalGate ? 0b0000 : 0b1111));
        }
    }
    ...
    void findMonoStep() {
        ...
        for (unsigned int blockOffset = 0; blockOffset < SIZE / BLOCK_SIZE;
             blockOffset++) {
            ...
            for (unsigned int stepInBlockOffset = 0;
                 stepInBlockOffset < BLOCK_SIZE;
                 stepInBlockOffset++) {
                unsigned int stepInBlockIdx =
                    BLOCK_SIZE - stepInBlockOffset - 1;
                int maskToCompare = 1 << stepInBlockIdx;
                if (blockMask & maskToCompare) {
                    this->hasActiveStep = true;
                    this->activeStepIdx =
                        blockIdx * BLOCK_SIZE + stepInBlockIdx;
    ...
    void applyMutations(unsigned int attrIdx, unsigned int blockIdx) {
        ...
        this->stepBasesMutated[attrIdx][blockIdx] = simd::clamp(
            this->stepBases[attrIdx][blockIdx]
    ...
    void recalcInOuts(unsigned int blockIdx) {
        this->stepInsComputed[blockIdx] = eucMod(
            this->stepIns[blockIdx]
                + this->stepBases[StepAttr::STEP_SHIFT][blockIdx] + this->shift
                + this->globalShift,
            1.f);
        ...
        this->stepMutaInsComputed[blockIdx] = eucMod(
            this->stepIns[blockIdx]
                + this->stepBasesMutated[StepAttr::STEP_SHIFT][blockIdx]
                + this->shift + this->globalShift,
            1.f);

        this->stepMutaOutsComputed[blockIdx] = eucMod(
            this->stepMutaInsComputed[blockIdx]
                + this->stepBasesMutated[StepAttr::STEP_LEN][blockIdx]
                      * this->globalLen,
            1.f);
    }
    ...
    json_t* dataToJson() {
        ...
        for (unsigned int stepIdx = 0; stepIdx < SIZE; stepIdx++) {
            ...
            json_object_set_new(
                ...
                "gate",
                json_boolean(bool(
                    simd::movemask(this->stepGates[blockIdx])
                    & (1 << stepInBlockIdx))));

            json_t* attrsJ = json_array();

            for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL;
                 attrIdx++) {
            ...
            json_object_set_new(
                stepJ,
                "in_",
                json_real(stepIdx * this->baseStepLen));
    ...
    void dataFromJson(json_t* patternJ, bool compatibilityMode) {
        this->resolution =
            json_number_value(json_object_get(patternJ, "resolution"));
    ...
    void randomize() {
        ...
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE;
            ...
            for (unsigned int stepInBlockIdx = 0; stepInBlockIdx < BLOCK_SIZE;
                 stepInBlockIdx++) {
    ...
    void quantize() {
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE;
            ...
            this->stepBases[StepAttr::STEP_SHIFT][blockIdx] =
                simd::trunc(
                    this->stepBases[StepAttr::STEP_SHIFT][blockIdx] * SIZE * 2)
                / SIZE * 2;
            this->stepMutas[StepAttr::STEP_SHIFT][blockIdx] =
                simd::trunc(
                    this->stepMutas[StepAttr::STEP_SHIFT][blockIdx] * SIZE * 2)
                / SIZE * 2;
...
public:
    ...
    void
        mutateBlock(unsigned int blockIdx, simd::float_4 mask, float factorScalar) {
        for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL; attrIdx++) {
            std::pair<float, float> bounds = this->attrBounds[attrIdx];
            ...
            simd::float_4 mutatedNormalized =
                (basesMutated - bounds.first) / range * 2.f - 1.f;
            simd::float_4 overflowCaution =
                simd::trunc(mutatedNormalized * 2.f);
            ...
            *vctrs = simd::ifelse(
            ...
            this->stepMutas[attrIdx][blockIdx] =
                simd::ifelse(mask, newMutas, mutas);
            this->applyMutations(attrIdx, blockIdx);
            this->recalcInOuts(blockIdx);
        }
    }
    ...
    void reverse() {
        ...
        for (unsigned int blockIdx = 0; blockIdx < SIZE / BLOCK_SIZE;
             blockIdx++) {
            std::swap(
                this->stepBases[StepAttr::STEP_EXPR_IN][blockIdx],

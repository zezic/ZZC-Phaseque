...
struct Mutator {
    ...
    float mutate(float value, float force, float low, float high) {
        ...
        this->factor = clamp(
            this->factor + (random::uniform() - 0.5f) * 0.05f,
            -1.0f,
            1.0f);
...
struct MutableValue {
    ...
    json_t* dataToJson() {
        ...
        json_object_set_new(
            mutableValueJ,
            "mutation",
            json_real(this->mutation));
        json_object_set_new(
            mutableValueJ,
            "isClean",
            json_boolean(this->isClean));
    ...
    void dataFromJson(json_t* mutableValueJ) {
        ...
        this->mutation =
            json_number_value(json_object_get(mutableValueJ, "mutation"));
        this->isClean =
            json_boolean_value(json_object_get(mutableValueJ, "isClean"));
        this->applyMutation();
    }
    ...
    void mutate(float factor) {
        ...
        this->isClean =
            this->mutation == 0.0f && this->base == this->defaultValue;
    }
    ...
    void adjValue(float factor) {
        if ((factor > 0.0f && this->base == this->maxValue)
            ...
            // base is stuck to boundary, try to achieve target value by reducing
            // mutation
            this->mutation += factor;
        } else {
            this->base =
                clamp(this->base + factor, this->minValue, this->maxValue);
        }

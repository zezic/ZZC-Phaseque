...
struct MainDisplayConsumer {
    ...
    MainDisplayConsumer() { this->pattern.init(); }
};
...
struct MainDisplayWidget : BaseDisplayWidget {
    ...
    void drawExprCurve(const DrawArgs &args,
                       float           x1,
                       float           workArea,
                       ...
                       bool            mutated)
    {
        ...
        if (mutated) {
            ...
            mutationStrength
                += std::abs(this->consumer->pattern
                                .stepMutas[StepAttr::STEP_EXPR_IN][blockIdx][stepInBlockIdx]);
            mutationStrength
                += std::abs(this->consumer->pattern
                                .stepMutas[StepAttr::STEP_EXPR_OUT][blockIdx][stepInBlockIdx]);
            mutationStrength
                += std::abs(this->consumer->pattern
                                .stepMutas[StepAttr::STEP_EXPR_POWER][blockIdx][stepInBlockIdx]);
            mutationStrength
                += std::abs(this->consumer->pattern
                                .stepMutas[StepAttr::STEP_EXPR_CURVE][blockIdx][stepInBlockIdx]);
        ...
        float len = workArea
                    * ((mutated ? this->consumer->pattern.stepBasesMutated :
                                  this->consumer->pattern
                                      .stepBases)[StepAttr::STEP_LEN][blockIdx][stepInBlockIdx])
    ...
    void drawStep(const DrawArgs &args, unsigned int stepIdx, bool mutated)
    {
        ...
        if (mutated) {
            ...
            mutationStrength += std::abs(
                this->consumer->pattern.stepMutas[StepAttr::STEP_VALUE][blockIdx][stepInBlockIdx]);
            mutationStrength += std::abs(
                this->consumer->pattern.stepMutas[StepAttr::STEP_LEN][blockIdx][stepInBlockIdx]);
            mutationStrength += std::abs(
                this->consumer->pattern.stepMutas[StepAttr::STEP_SHIFT][blockIdx][stepInBlockIdx]);
        ...
        float x2 = padding
                   + workArea
                         * (mutated ?
                                this->consumer->pattern.stepMutaOutsComputed :
        ...
        float y = padding + crossLine2
                  + -stepY
                        * (mutated ? this->consumer->pattern.stepBasesMutated :
                                     this->consumer->pattern.stepBases)[StepAttr::STEP_VALUE]
    ...
    void drawSteps(const DrawArgs &args)
    {
        ...
        for (unsigned int stepIdx = 0; stepIdx < this->consumer->pattern.size; stepIdx++) {
            ...
            bool gate = bool(simd::movemask(this->consumer->pattern.stepGates[blockIdx])
                             & (1 << stepInBlockIdx))
            ...
            if (this->consumer->polyphonyMode == PolyphonyModes::MONOPHONIC) {
                ...
                active
                    = simd::movemask(this->consumer->pattern.hits[blockIdx]) & 1 << stepInBlockIdx;
            } else if (this->consumer->polyphonyMode == PolyphonyModes::UNISON) {
                active = simd::movemask(this->consumer->pattern.hitsClean[blockIdx])
                         & 1 << stepInBlockIdx;
            }
        ...
        //       std::min(this->consumer->activeStep.mutationStrength, 1.f)); drawStep(args,
        //       &this->consumer->activeStep, true);
        //     }
        //   }

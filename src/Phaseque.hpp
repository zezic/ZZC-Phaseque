...
struct Phaseque : Module {
    ...
    struct StepParamConfigurator {
        void operator()(Module* module) {
            module->configParam(
                GATE_SWITCH_PARAM + size,
                0.0f,
                1.0f,
                0.0f,
                "Step Gate");
            module->configParam<StepAttrParamQuantity<size, STEP_VALUE>>(
                STEP_VALUE_PARAM + size,
                -2.0f,
                2.0f,
                0.0f,
                "Step Value");
            module->configParam<StepAttrParamQuantity<size, STEP_LEN>>(
                ...
                1.0f / NUM_STEPS,
                "Step Length");
            module->configParam<StepAttrParamQuantity<size, STEP_SHIFT>>(
                STEP_SHIFT_PARAM + size,
                -1.0f / NUM_STEPS,
                1.0f / NUM_STEPS,
                0.0f,
                "Step Shift");
            module->configParam<StepAttrParamQuantity<size, STEP_EXPR_IN>>(
                STEP_EXPR_IN_PARAM + size,
                -1.0f,
                1.0f,
                0.0f,
                "Step Expression In");
            ...
            module->configParam<StepAttrParamQuantity<size, STEP_EXPR_OUT>>(
                STEP_EXPR_OUT_PARAM + size,
                -1.0f,
                1.0f,
                0.0f,
                "Step Expression Out");
    ...
    Phaseque() {
        ...
        configParam(
            ...
            1.0f,
            0.0f,
            "Sync inter-beat phase with clock");
        configParam(
            BPM_PARAM,
            0.0,
            240.0,
        ...
        configParam(
            ...
            1.0f,
            0.0f,
            "Absolute Phase Input");
        configParam(
            CLUTCH_SWITCH_PARAM,
            0.0f,
            1.0f,
            0.0f,
            "Clutch Transport with Phase");
        ...
        configParam(
            ...
            1.0f,
            0.0f,
            "Disable CV-driven Pattern Navigation");
        ...
        configParam(
            ...
            2.0,
            1.0,
            "Global Step Length Modifier");
        configParam(
            SHIFT_LEFT_SWITCH_PARAM,
            0.0f,
            1.0f,
            0.0f,
            "Shift Pattern to the Left by 1/8");
        configParam(
            SHIFT_RIGHT_SWITCH_PARAM,
            0.0f,
            1.0f,
        ...
        configParam(
            ...
            1.0f,
            0.0f,
            "Quantize Steps Position");
        configParam(
            LEN_SWITCH_PARAM,
            0.0f,
            1.0f,
            0.0f,
            "Quantize Steps Length");
        configParam(REV_SWITCH_PARAM, 0.0f, 1.0f, 0.0f, "Reverse Pattern");
        configParam(FLIP_SWITCH_PARAM, 0.0f, 1.0f, 0.0f, "Flip Pattern");
        configParam<PatternResoParamQuantity>(
            PATTERN_RESO_PARAM,
            1.0f,
            99.0f,
            8.0f,
            "Pattern Resolution");
        configParam<PatternShiftParamQuantity>(
            PATTERN_SHIFT_PARAM,
            -1.0f,
            1.0f,
            0.0f,
            "Pattern Shift");
        configParam<PatternMutaParamQuantity>(
            PATTERN_MUTA_PARAM,
            -INFINITY,
            INFINITY,
            0.0f,
        ...
        configInput(
            SEQ_INPUT,
            "Advance to Next Pattern (Including Unmodified) Trigger");
        ...
        configInput(
            GLOBAL_EXPR_CURVE_INPUT,
            "Global Expression Curve CV (±5V)");
        configInput(
            GLOBAL_EXPR_POWER_INPUT,
            "Global Expression Power CV (±5V)");

        for (size_t i = 0; i < NUM_STEPS; i++) {
            configInput(
                STEP_JUMP_INPUT + i,
                "Jump to Step " + std::to_string(i + 1) + " Trigger");
        }
        ...
        for (size_t i = 0; i < NUM_STEPS; i++) {
            configOutput(
                STEP_GATE_OUTPUT + i,
                "Step " + std::to_string(i + 1) + " Active Gate");
        }
        ...
        configOutput(
            PTRN_WRAP_OUTPUT,
            "Pattern Wrap (Current Pattern Restarted) Trigger");
    ...
    void renderParamQuantities() {
        this->paramQuantities[PATTERN_RESO_PARAM]->setValue(
            this->pattern.resolution);
        this->paramQuantities[PATTERN_SHIFT_PARAM]->setValue(
            this->pattern.shift / this->pattern.baseStepLen);
        for (unsigned int stepIdx = 0; stepIdx < this->pattern.size;
             stepIdx++) {
            ...
            for (unsigned int attrIdx = 0; attrIdx < STEP_ATTRS_TOTAL;
                 attrIdx++) {
                paramQuantities
                    [STEP_VALUE_PARAM + stepIdx + attrIdx * this->pattern.size]
                        ->setValue(this->pattern.stepBases[attrIdx][blockIdx]
                                                          [stepInBlockIdx]);
            }
        }
    }
    ...
    json_t* dataToJson() override {
        ...
        json_object_set_new(
            rootJ,
            "globalGateInternal",
            json_boolean(globalGateInternal));
        json_object_set_new(
            rootJ,
            "patternIdx",
        ...
        json_object_set_new(
            rootJ,
            "polyphonyMode",
            json_integer(polyphonyMode));
        json_object_set_new(
            rootJ,
            "useCompatibleBPMCV",
            json_boolean(useCompatibleBPMCV));
    ...
    void dataFromJson(json_t* rootJ) override {
        ...
        json_t* globalGateInternalJ =
            json_object_get(rootJ, "globalGateInternal");
        ...
        json_t* useCompatibleBPMCVJ =
            json_object_get(rootJ, "useCompatibleBPMCV");
        ...
        if (useCompatibleBPMCVJ) {
            ...
            useCompatibleBPMCV =
                false; // Fallback to pre v1.1.3 default behavior
        ...
        if (patternsJ) {
            size_t arraySize = json_array_size(patternsJ);
            int offset =
                arraySize - NUM_PATTERNS; // For compatibility with v1.1.2
            // we will take 32 last arrays
            for (unsigned int i = offset;
                 i < arraySize && (i - offset) < NUM_PATTERNS;
                 i++) {
                json_t* patternJ = json_array_get(patternsJ, i);
                if (!json_is_null(patternJ)) {
                    this->patterns[i - offset].dataFromJson(
                        patternJ,
                        compatibilityMode);
                }
            }

...
struct Step {
    ...
    float outBase() { return inBase() + minLenBase(); }
    ...
    float fastIn(float shift) { return in_ + attrs[STEP_SHIFT].value + shift; }
    ...
    void init(int i)
    {
        ...
        for (int i = 0; i < STEP_ATTRS_TOTAL; i++) {
            ...
            this->attrs[i].setup(defaults.defaultValue,
                                 defaults.minValue,
                                 defaults.maxValue,
                                 defaults.mutMult);
            this->attrs[i].init();
    ...
    void randomize()
    {
        // Random uniform with a low threshold used because it's too boring when there is only few
        // notes
    ...
    {
        return curve(phase,
                     clamp(this->attrs[STEP_EXPR_CURVE].value
                               + (exprCurvePort ? exprCurvePort->getVoltage() * 0.2f : 0.0f),
                           -1.0f,
                           1.0f),
                     clamp(this->attrs[STEP_EXPR_POWER].value
    ...
    {
        return curve(phase,
                     clamp(this->attrs[STEP_EXPR_CURVE].base
                               + (exprCurvePort ? exprCurvePort->getVoltage() * 0.2f : 0.0f),
                           -1.0f,
                           1.0f),
                     clamp(this->attrs[STEP_EXPR_POWER].base
                               + (exprPowerPort ? exprPowerPort->getVoltage() * 0.2f : 0.0f),
                           -1.0f,
                           1.0f),
    ...
    void quantize()
    {
        this->attrs[STEP_SHIFT].setValue(
            roundf(this->attrs[STEP_SHIFT].value * (1.0f / baseStepLen) * 2.0f) * baseStepLen
            * 0.5f);
    }

    void resetLength() { this->attrs[STEP_LEN].init(); }
    ...
    void updateIn() { this->in_ = this->idx * baseStepLen; }

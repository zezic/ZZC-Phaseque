...
struct ZZC_PhasequePatternResoKnob : SvgKnob {
    ...
    ZZC_PhasequePatternResoKnob() {
        setSvg(APP->window->loadSvg(
            asset::plugin(pluginInstance, "res/knobs/ZZC-Knob-25-Encoder.svg")));
    ...
    void onDragEnd(const event::DragEnd& e) override {
        ...
        if (paramQuantity) {
            ...
            if (this->oldValue != newValue) {
                ...
                Phaseque* phaseq =
                    static_cast<Phaseque*>(paramQuantity->module);
                h->patternNum = phaseq->patternIdx;
                h->oldValue = oldValue;
                h->newValue = newValue;
                h->name = "change pattern " + std::to_string(phaseq->patternIdx)
                          + " resolution";
                APP->history->push(h);
            }
        }
...
struct ZZC_DisplayKnob : SvgKnob {
    ...
    void recalcSizes() {
        ...
        shadow->box.pos = math::Vec(padding, padding)
                              .plus(math::Vec(0, sw->box.size.y * 0.2));
        disp->strokeWidth = strokeWidth;
        disp->box.size = fb->box.size;
    }
...
struct ZZC_PhasequeMutaKnob : SvgKnob {
    ZZC_PhasequeMutaKnob() {
        setSvg(APP->window->loadSvg(
            asset::plugin(pluginInstance, "res/knobs/ZZC-Knob-27-Encoder.svg")));
    ...
    void onDragEnd(const event::DragEnd& e) override {
        ...
        if (paramQuantity) {
            ...
            if (oldValue != newValue) {
                ...
                h->oldValue = oldValue;
                h->newValue = newValue;
                StepAttrParamQuantityBase* q =
                    static_cast<StepAttrParamQuantityBase*>(paramQuantity);
                h->step = q->item;
    ...
    void onDragEnd(const event::DragEnd& e) override {
        ...
        if (this->paramQuantityX) {
            ...
            if (this->oldValueX != newValueX) {
                ...
                Phaseque* phaseq =
                    static_cast<Phaseque*>(paramQuantityX->module);
                h->patternNum = phaseq->patternIdx;
                h->oldValue = oldValueX;
                h->newValue = newValueX;
                StepAttrParamQuantityBase* q =
                    static_cast<StepAttrParamQuantityBase*>(paramQuantityX);
                h->step = q->item;
                h->name = "change step " + StepAttrNames[q->attr];
                h->attr = q->attr;
                APP->history->push(h);
        ...
        if (this->paramQuantityY) {
            ...
            if (this->oldValueY != newValueY) {
                ...
                Phaseque* phaseq =
                    static_cast<Phaseque*>(paramQuantityY->module);
                h->patternNum = phaseq->patternIdx;
                h->oldValue = oldValueY;
                h->newValue = newValueY;
                StepAttrParamQuantityBase* q =
                    static_cast<StepAttrParamQuantityBase*>(paramQuantityY);
                h->step = q->item;
                h->name = "change step " + StepAttrNames[q->attr];
                h->attr = q->attr;
                APP->history->push(h);
...
struct ZZC_PhasequeStepAttrKnob23 : ZZC_PhasequeAttrKnob {
    ZZC_PhasequeStepAttrKnob23() {
        setSvg(APP->window->loadSvg(asset::plugin(
            pluginInstance,
            "res/knobs/ZZC-Knob-27-23-Encoder.svg")));
        shadow->box.size = Vec(33, 33);
...
struct ZZC_PhasequeStepAttrKnob21 : ZZC_PhasequeAttrKnob {
    ZZC_PhasequeStepAttrKnob21() {
        setSvg(APP->window->loadSvg(asset::plugin(
            pluginInstance,
            "res/knobs/ZZC-Knob-27-21-Encoder.svg")));
        shadow->box.size = Vec(29, 29);
...
struct ZZC_PhasequeStepAttrKnob19 : ZZC_PhasequeAttrKnob {
    ZZC_PhasequeStepAttrKnob19() {
        setSvg(APP->window->loadSvg(asset::plugin(
            pluginInstance,
            "res/knobs/ZZC-Knob-27-19-Encoder.svg")));
